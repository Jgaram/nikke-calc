...
| `not_reloading` | 발동 시점 | ✅ 러스트 / — 파이썬 | 시전자가 재장전 중이면 거짓. 러스트 코어가 타임라인의 `reloading_until`을 `state.reloading`으로 비춰 판정한다. 아스카 : WILLE 긴급 수복 3·5 전용(2026-09-04). 파이썬은 모르는 조건이라 무시(항상 참). |
| `reloading` | 발동 시점 | ✅ 러스트 / — 파이썬 | 위의 반대. 지금은 쓰는 데이터가 없다(예열 유지 가설을 넣었다 뺌, 2026-09-04). |
