...
_AREA_LABEL = {83: "한섭", 81: "일섭", 84: "글로벌섭", 91: "대만·홍콩·마카오"}
...
# 대만·홍콩·마카오(91)는 유저가 블라링크 요청에서 뽑아 알려 준 값이다(2026-09-02,
# `nikke_area_id=91` · `area_id=tw` · `X-Language=zh-TW`).
#
# **넷을 다 훑는다.** 한 계정에 지역이 여럿 걸리는 일이 실제로 있어서(한섭+일섭 실측),
# 앞에서 하나 나왔다고 멈추면 나머지 지역은 영영 안 보인다 — 한섭+대만도 마찬가지다.
# 요청 하나가 느는 값(0.2초 안팎)보다 «내 계정이 안 나온다»가 비싸다(유저 결정).
_AREA_CANDIDATES = (83, 81, 84, 91)
