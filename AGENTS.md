...
## 저장소 구도 (2026-08-26 분리)

이 저장소는 **파이썬 계산기·스크래핑 도구·데이터 정본·파이썬 사이드카(`web/server.py`)**다.
사이트(프론트+본체 서버)와 계산 코어는 각자 별도 저장소로 분리됐다. `data/`는 여기가 정본이고
사이트 저장소가 사본을 쓴다 — 데이터를 갱신하면 그쪽에 복사해 준다. 웹 UI·다국어 규칙은
사이트 저장소의 `AGENTS.md`에 있다.
...
| 사이트 구조·배포·서버 계약 | 사이트 저장소의 `SITE.md`·`context/SERVER-CONTRACT.md` |
| 계산기 감사 결과 (의심 지점·검증 항목) | `context/CALC-AUDIT.md` |
| 사이트가 데이터를 어떻게 다루는가 | 사이트 저장소의 `PRIVACY.md` |
