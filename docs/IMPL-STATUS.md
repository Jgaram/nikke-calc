...
| `explosion_range` | `explosion_range` | — | ✅ | 폭발 범위 ▲(%). **대미지 식에는 안 들어간다** — `get_buffs`가 합산하고, 보스 패턴 단계 모드에서는 발사체 폭발이 닿는 파츠·저지원 위치 단계를 가르고(합 100% 이상이면 4단계 표적까지 — `boss_pattern.hit_reach`), **좌표 모드에서는 폭발 원의 반지름을 ×(1 + N%)** 한다(`CharState._area_radius` — CDN `spot_explosion_range` × `explosion_scale`). 네온 : 비전 아이 · 라플라스 · 라피 : 레드 후드 · 베스티 : 택티컬 업 · 아니스 : 스타 |
| `pierce_range` | `pierce_range` | — | ✅ | 관통 범위 ▲(%). `explosion_range`와 같은 자리 — 단계 모드는 관통이 닿는 단계를 가르고(합 100% 이상이면 2단계 파츠·저지원까지), 좌표 모드는 관통 원(착탄점 주변, 기본 `pierce_px` 25px ⬜)의 반지름을 ×(1 + N%) 한다. 도로시 : 세렌디피티 · 레드 후드 |
