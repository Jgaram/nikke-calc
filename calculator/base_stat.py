...
def calc_base_stats(char: dict) -> dict:
    ...
    # 큐브 플랫 스탯. **레벨 0 = 미장착**이다 — 표는 1~15만 갖고 있다.
    # 예전에는 0이 KeyError라 «큐브를 안 낀 상태»를 아예 표현할 수 없었고, 그래서
    # 프로필이 전원 렐릭 베어 Lv15로 계산됐다. 큐브 공통 스킬 `안티 코드 HC`가
    # 우월 코드 대미지를 최대 +19.09% 주므로, 실제로 안 낀 니케의 우코 딜이 크게
    # 부풀었다(2026-08-24 실측 대조에서 드레이크 +17%p 과대평가의 원인).
    # 스킬 쪽(`buff_manager._make_cube_effects`)은 레벨에 해당 값이 없으면 이미
    # 아무 효과도 붙이지 않으므로, 여기서 플랫 스탯만 0으로 막으면 된다.
    cube_s = _CUBE["_stats"].get(str(cube_inst["level"])) or _zero()
