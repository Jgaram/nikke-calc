...
변환 규칙만 바뀌었을 때는 재수집이 필요 없다 — `--from-raw`가 `profiles/<이름>.raw.json`으로
...
| 오버로드 **부위 경계** (`_overload`) | ○ | 계산에는 안 들어가는 `_` 키다(합산 쪽이 정본). 어느 줄이 어느 부위인지는 더 굴릴지 판단할 때 필요하다 — 굴릴 부위는 배경에서 빼야 하므로(`overload/README.md` §굴릴 부위) 웹앱 오버로드 화면이 이걸 읽는다 |
