...
4. **선례 조회 도구를 쓸 준비**: `python -m runner.precedent "<clause 원문>"`. 파싱 중 문구→키 매핑이 조금이라도 갈리면 규칙을 다시 읽기 전에 이것부터 돌린다 — 파싱을 마친 캐릭터 전원의 원문이 이미 색인돼 있어 대부분 답이 나온다. 재구현이면 `--exclude <대상 캐릭터>`로 자기 자신을 뺀다.
