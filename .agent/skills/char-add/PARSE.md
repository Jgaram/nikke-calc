...
1. `docs/PARSING.md` **§1~8 읽는다** (§9 예외·§12 현황은 `PARSING-CHARS.md`로 분리 — 필요 시 참조).
   §10 「유저에게 물어봐야 할 시점」은 기본 모드에서 **아래 4의 선례 조회와 `SCENARIO.md`
   Step 6 §판단 순서를 거친 뒤에도 남는 것**에 적용된다 — §10-0의 `_unparseable`과
   §10-5 값 불일치는 그 자체로 즉시 질문이고, 나머지 「불명확」 항목은 선례가 답하면
   해소된 것이다. 단계별 모드는 §10을 문자 그대로 따른다.
...
7. `python -m runner.roster` 실행 — 파싱 현황 로스터(루트 `roster.html`) 재생성. 데이터에서 파생되므로 HTML은 손대지 않는다.
