...
3. **선례 대조** — 선례로 해소한 항목은 근거 캐릭터 이름을 함께 적는다. 유저가 한 줄로 검증할 수 있어야 한다.
   선례와 어긋나거나 선례가 없는 자리는 `python -m runner.parsecheck "<이름>"`(`PARSE.md` Phase A 7)의
   **`선례와 다름`·`선례 없음` 목록을 그대로** 붙인다. `선례 없음`이 AI가 스스로 판단한 자리 전부다 — 유저 검토는 거기서 시작한다.
