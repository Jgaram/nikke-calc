...
현재 코드의 기존 버그도 "정상"으로 고정된다. 정확성은 docs/scenarios/*.md 담당.
    ...
    python -m runner.snapshot --squad 스쿼드1   # 일부만
...
SQUADS: dict[str, dict] = {
    ...
    "레이드_트리나홍련": {
        ...
        # 코어 직경 52px는 `docs/mechanics/명중률 탄착군.md`의 추정 코어 반경 26px에서
    ...
    "지그_아스카": {
        ...
        # 실전 조합 자료가 없어 지그로 둔다. 시나리오는 `docs/scenarios/아스카.md`.
        #
    ...
    "지그_아스카풍압코어": {
        ...
        # 코어 직경 52px는 `레이드_트리나홍련`과 같은 값이다(`docs/mechanics/명중률 탄착군.md`).
        "members": ["리틀 머메이드", "크라운", "아스카", "test_B3"],
        "config": {"first_burst_time": 3.0},
    ...
    "컨트롤_미란다미하라": {
        # 커버: 버스트 엄폐컨 own_full_burst (docs/CONTROL.md §버스트 엄폐컨)
    ...
    "컨트롤_에이다미하라": {
        # 커버: 홀드컨 own_full_burst (docs/CONTROL.md §홀드)
        ...
        # (`_control_rules`) — 컨트롤을 끈 대조군은 `runner/sim.py --auto`로 본다.
        "members": ["미란다", "그레이브", "에이다", "미하라 : 본딩 체인", "D : 킬러 와이프"],
        "config": {"no_burst_char": "D : 킬러 와이프", "first_burst_time": 3.0},
        "enemy": {"code": "풍압"},
        "seed": 42,
