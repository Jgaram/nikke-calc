...
#   장전컨 A `before_fb_end`   기본형 `S40_브리드라피` · `if_dry` `S37_브래디퀀시` ·
#                             `duration` `S39_나가라피`
...
SQUADS: dict[str, dict] = {
    ...
    "S37_브래디퀀시": {
        ...
        # 장전컨 `if_dry` 자리 — 정책 A인데 **남은 장탄으로 다음 풀버스트까지 못 버틸 때만**
        # 엄폐한다. 늘 엄폐하는 쪽은 `S40_브리드라피`가 덮으므로 둘이 대조 쌍이다.
        "members": ["마스트 : 로망틱 메이드", "퀀시 : 이스케이프 퀸", "앵커 : 이노센트 메이드",
                    "브래디", "라피 : 레드 후드"],
        "chars": {
            "라피 : 레드 후드": {
                "control": {"reload": {"policy": "before_fb_end", "lead": 0.3, "if_dry": True}},
            },
        },
        "config": {"first_burst_time": 3.0},
        "enemy": {"code": "작열"},
    ...
    "S39_나가라피": {
        ...
        # 격 사이클을 준다.
        # 장전컨 `duration` 자리 — 엄폐를 재장전 완료까지가 아니라 **고정 1.0초**만 연다.
        # 값을 안 주는 쪽(`S40_브리드라피`)과 대조 쌍이다.
    ...
    "S40_브리드라피": {
        ...
        # 라피 : 레드 후드 장전컨(정책 A) 자리 — 비버스트에 재장전이 걸리지 않도록
        # 풀버스트가 끝나기 전에 미리 채운다. 옵션 없는 기본형이고,
        # `if_dry`는 `S37_브래디퀀시`, `duration`은 `S39_나가라피`가 덮는다.
